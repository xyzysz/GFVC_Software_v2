...
if __name__ == "__main__":
    ...
    if opt.dataset_config is not None:
        ...
        yuv_seqdir=dataset_args['yuv_sequence_dir'] 
    else:
        seqlist= args['seq_list'] 
        Sequence_dir=args['sequence_dir'] 
        
        yuv_seqdir=args['yuv_sequence_dir'] 
    ...
    metric = {'psnr': 1, 'ssim': 1}
    if metric['psnr']:
        totalResult_PSNR=np.zeros((len(seqlist)+1,len(qp_iter)))
        totalResult_Y_PSNR=np.zeros((len(seqlist)+1,len(qp_iter)))        
        totalResult_U_PSNR=np.zeros((len(seqlist)+1,len(qp_iter)))               
        totalResult_V_PSNR=np.zeros((len(seqlist)+1,len(qp_iter)))               
        
    if metric['ssim']:
        totalResult_SSIM=np.zeros((len(seqlist)+1,len(qp_iter)))
        totalResult_Y_SSIM=np.zeros((len(seqlist)+1,len(qp_iter)))        
        totalResult_U_SSIM=np.zeros((len(seqlist)+1,len(qp_iter)))          
        totalResult_V_SSIM=np.zeros((len(seqlist)+1,len(qp_iter)))          
    ...
    for seq in seqlist:
        ...
        for qp in qp_iter:
            ...
            original_video = yuv_seqdir +testingdata_name+'_'+str(seq)+'_'+str(width)+'x'+str(height)+'_25_8bit_420.yuv'

            if Model in ['HDAC']:
                encoded_video = '../experiment/'+Model+'/'+'Iframe_'+Iframe_format+'/dec_yuv420/'+testingdata_name+'_'+str(seq)+'_256x256_25_8bit_420_qp'+str(qplist[0])+'_bqp'+str(qp)+'.yuv'
            elif Model in ['HDAC_Extension']:
                encoded_video = '../experiment/'+Model+'/'+'Iframe_'+Iframe_format+'/dec_yuv420/'+testingdata_name+'_'+str(seq)+'_256x256_25_8bit_420_qp'+str(qp)+'_bqp'+str(bl_qplist[0])+'.yuv'
            else:            
            ...
            print(Model+'_'+testingdata_name+'_'+str(seq)+'_QP'+str(qp)+'.yuv',"success. Time is %.4f"%(end-start))
            qpIdx+=1
        seqIdx+=1
